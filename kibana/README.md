...
**项目增强**：仪表盘配置已实现“配置即代码”，可通过版本化管理，一键在新环境部署恢复。
...
运行 `web-app` 文件夹中的 `stress_test.py`，产生访问日志数据：
```bash
cd web-app
...
### **【新增】仪表盘配置导入（首次部署必做）**
服务启动后（约等待1分钟），需要将项目预置的仪表盘配置导入Kibana：
...
> **部署提示**：在生产环境中，请将 `localhost` 替换为实际服务器地址或域名。

### 相关服务访问
- **Kibana主页**：http://localhost:5601
- **Elasticsearch API**：http://localhost:9200
- **Web应用示例**：http://localhost:8000
- **Logstash监控**：http://localhost:9600

---

## 🔄 **【新增】仪表盘配置管理**

本项目已包含完整的仪表盘配置（`.ndjson`文件），实现了配置的版本化管理。

### 导出配置（用于备份或修改后保存）
若你在Kibana中修改了仪表盘并希望保存更改：
1.  在Kibana进入 **`Stack Management > Saved Objects`**。
2.  勾选你的仪表盘对象（如 `Web应用监控中心`）。
3.  点击 **`Export`（导出）**，下载 `.ndjson` 文件。
4.  用新文件替换项目 `kibana/saved_objects/` 目录下的旧文件，并提交至Git。

### 导入配置（部署与恢复）
（步骤同上方“快速访问”部分，此处略）
通过此方式，可在任何新部署的ELK环境中快速复现完全一致的监控视图。
